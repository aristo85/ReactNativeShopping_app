  ...
  return async (dispatch) => {
    try {
      const response = await fetch(
        `https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyCggC0E3Na99L_czdyhsBt97A7-PXDPrdY`,
        {
          method: "POST",
          headers: {
...
export const loginUser = (authData) => {
  return async (dispatch) => {
    try {
      const response = await fetch(
        `https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyCggC0E3Na99L_czdyhsBt97A7-PXDPrdY`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
