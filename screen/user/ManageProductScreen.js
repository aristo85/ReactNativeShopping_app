...
const ManageProductScreen = (props) => {
  ...
  useEffect(() => {
    const willFocusSub = props.navigation.addListener("willFocus", () => {		//see documentation for these kind
      dispatch(getData());								//of navigation listeners.
    });
    return () => {					//need to clean listener everytime
      willFocusSub.remove();
    }
  }, [dispatch]);
  ...
  if (myProducts.length < 1) {
    return (
      <View style={styles.spiner}>
        <TextTitle>No Product of your among the list, may be you could create one!</TextTitle>
      </View>
    );
  }
...
ManageProductScreen.navigationOptions = (navData) => {
  return {
