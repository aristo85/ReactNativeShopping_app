...
const EditProductScreen = (props) => {
  ...
  const priceParam = props.navigation.getParam("price");
  //initiate useReducer props
  const [formState, dispatchFormSatate] = useReducer(formReducer, {
    inputValues: {
      title: priceParam ? props.navigation.getParam("title") : "",
      imgUrl: priceParam ? props.navigation.getParam("imageUrl") : "",
      price: priceParam ? String(props.navigation.getParam("price")) : "",
      description: priceParam ? props.navigation.getParam("description") : "",
    },
    formErrors: [],
  });
  ...
  const handleSave = (dataInputs) => {
    ...
    schema.isValid(dataInputs).then((valid) => {
      ...
      if (isValid) {
        priceParam
          ? dispatch(onEditSave({...dataInputs, id: props.navigation.getParam("id")}))
    ...
    useEffect(() => {
    const dataInputs = {
      ownerId: priceParam ? props.navigation.getParam("ownerId") : "u1",
...
EditProductScreen.navigationOptions = (navData) => {
  return {
    headerTitle: navData.navigation.getParam("price") ? "Edit" : "Create",
    headerRight: () => (
      <HeaderButtons HeaderButtonComponent={HeaderButtonStyle}>
        <Item
          ...
          onPress={() => navData.navigation.getParam("save")()} //connection with redux
        />
      </HeaderButtons>
    ),
